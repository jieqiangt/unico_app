...
export function formatLineitems(lineItems, inputs, outputs) {
  const formattedLineItems = lineItems.map((lineItem) => {
    const lineId = lineItem.lineId;
    const lineInputs = inputs.filter((input) => input.refLineId == lineId);
    const lineOutputs = outputs.filter((output) => output.refLineId == lineId);
    return { ...lineItem, inputs: lineInputs, outputs: lineOutputs };
  });

  return JSON.parse(JSON.stringify(formattedLineItems));
}

export function formatToDateTimeStr(dateTimeStr) {
